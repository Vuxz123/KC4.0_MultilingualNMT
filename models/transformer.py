...
# from utils.save import load_model_from_path, check_model_in_path, save_and_clear_model, write_model_score,
# load_model_score, save_model_best_to_path, load_model
...
class Transformer(nn.Module):
    ...
    def __init__(self, mode=None, model_dir=None, config=None):
        ...
        if mode == "train":
            # training flow, necessitate the DataLoader and iterations. This will attempt to load vocab file from the
            # dir instead of rebuilding, but can build a new vocab if no data is found
        ...
        # get the maximum amount of tokens per sample in encoder. This is useful due to PositionalEncoder requiring
        # this value
        ...
        # load the beamsearch obj with preset values read from config. ALWAYS require the current model, max_length,
        # and device used as per DecodeStrategy base
        ...
        for epoch in range(checkpoint_idx, opt['epochs']):
            ...
            for i, batch in enumerate(self.train_iter):
                ...
                if (i + 1) % opt['printevery'] == 0:
                    ...
                    et = time.time() - s
                    # print('epoch: {:03d} - iter: {:05d} - train loss: {:.4f} - time elapsed/per batch: {:.4f} {
                    # :.4f}'.format(epoch, i+1, avg_loss, et, et / opt['printevery']))
                    logging.info(
